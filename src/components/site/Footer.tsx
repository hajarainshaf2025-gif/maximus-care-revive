...
function TikTokIcon() {
  return (
    <svg className="h-4 w-4" viewBox="0 0 24 24" fill="currentColor">
      <path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-2.88 2.5 2.89 2.89 0 0 1-2.89-2.89 2.89 2.89 0 0 1 2.89-2.89c.28 0 .54.04.79.1V9.01a6.27 6.27 0 0 0-.79-.05 6.34 6.34 0 0 0-6.34 6.34 6.34 6.34 0 0 0 6.34 6.34 6.34 6.34 0 0 0 6.33-6.34V8.93a8.16 8.16 0 0 0 4.77 1.52V7.01a4.85 4.85 0 0 1-1-.32z" />
    </svg>
  );
}
      ...
      <div className="container mx-auto px-6 py-16 grid gap-12 md:grid-cols-2 lg:grid-cols-4">
        <div>
          <div className="bg-white/95 inline-block rounded-lg p-3 mb-4">
            <img src={logo} alt="Maximus Care" className="h-20 w-auto" />
          </div>
          <p className="text-sm text-primary-foreground/80 leading-relaxed">
            MAXIMUS CARE PHYSIO AND REHAB UNIT - Sri Lanka's Trusted Physiotherapy & Rehabilitation
            Provider — Restoring movement, Independence and Quality of Life.
          </p>
          <div className="flex gap-2 mt-5">
            <a
              href={SITE.facebook}
              aria-label="Facebook"
              className="rounded-md border border-white/20 bg-white/5 p-2 hover:bg-white/15 hover:border-white/40 transition-all"
            >
              <FacebookIcon />
            </a>
            <a
            ...
            <a
              href="https://www.tiktok.com/@maximuscare.physi"
              aria-label="TikTok"
              target="_blank"
              rel="noopener noreferrer"
              className="rounded-md border border-white/20 bg-white/5 p-2 hover:bg-white/15 hover:border-white/40 transition-all"
          ...
          <ul className="space-y-3 text-sm text-primary-foreground/80">
            {BRANCHES.map((b) => (
              <li key={b.slug} className="flex gap-2">
                ...
                <span>
                  <strong className="text-primary-foreground block">{b.name}</strong>
                  {b.address}
                </span>
              </li>
            ))}
        ...
        <div>
          ...
          <ul className="space-y-3 text-sm text-primary-foreground/80">
            ...
            <li className="flex items-center gap-2">
              <WhatsAppIcon />{" "}
              <a href={`https://wa.me/${SITE.whatsapp2}`}>WhatsApp {SITE.whatsapp2Display}</a>
            </li>
            <li className="flex items-center gap-2">
