...
export const Route = createFileRoute("/testimonials")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Real recovery stories from patients we have treated across Sri Lanka — stroke, surgery, sports injuries, chronic pain and more." },
      ...
      <section className="py-16 container mx-auto px-6 grid md:grid-cols-2 gap-6">
        {TESTIMONIALS.map((t) => (
          <figure key={t.name} className="rounded-3xl bg-card border border-border p-8 shadow-soft hover:shadow-card transition-shadow">
            <div className="flex gap-0.5 text-accent">{Array.from({ length: t.stars }).map((_, i) => <Star key={i} className="h-5 w-5 fill-current" />)}</div>
            <blockquote className="mt-4 text-lg text-foreground/90 leading-relaxed">"{t.quote}"</blockquote>
